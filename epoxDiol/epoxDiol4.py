...
resultsHead=['Donor','Parent Fatty Acid','To-PP Ratio','To-F Ratio','NE-PP Ratio','NE-F Ratio']
results=[]
...
for pfa in parentFA:
    ...
    for i,donor in enumerate(donors):
        data=[donor,pfa,ToRatio_pp[i],ToRatio_f[i],NeRatio_pp[i],NeRatio_f[i]]
        results.append(data)



resultsDF=pd.DataFrame(results,columns=resultsHead)
resultsDF.to_csv('../results/epoxDiol4.csv',index=False)
