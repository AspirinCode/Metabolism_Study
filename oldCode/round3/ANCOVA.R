...
ANCOVA <- function(Sheet)
{
  ...
  final<-c('Name','fastingtoPP-pval','Pro vs Anti-pvalue','Grpandfasting-Pval') # creating a vector with heading name and P value 
      ...
      for (ind in matchInd)
      {
        ...
        if((as.character(name) != "%-C22:6n3-NEFA-PP") & (length(isPP) != 0))
        {
          ...
          if (Lm <= (L+1) & Lm>=(L-1) & L != Lm & vl==nl)
          {
            ...
            test <- aov(P~ Fa*factor(grp))
            test <- tidy(test)
            
            results <- c(sub, test$p.value[1],test$p.value[2],test$p.value[3]) 
