...
ANCOVA2 <- function(Sheet)
{
  
  MData   <- read_excel("~/Desktop/Metabolism_Study/MetData/Passerini TGRL-Plasma Final Res.xlsx", sheet = Sheet, col_names = FALSE)
  ...
  final<-c('Name','fa','grp') # creating a vector with heading name and P value 
      ...
      for (ind in matchInd)
      {
        ...
        if((as.character(name) != "%-C22:6n3-NEFA-PP") & (length(isPP) != 0))
        {
          ...
          if (Lm <= (L+1) & Lm>=(L-1) & L != Lm & vl==nl)
          {
            ...
            results <- c(sub, test$p.value[1],test$p.value[2]) 
